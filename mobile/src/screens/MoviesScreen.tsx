...
export function MoviesScreen() {
  ...
  const handleQuickAction = (actionId: string) => {
    ...
    switch (actionId) {
      case 'play':
        navigation.navigate('Media', { id: selectedItem.id })
        break
      case 'info':
        navigation.navigate('Media', { id: selectedItem.id })
        break
  ...
  const { data: movies, isLoading, refetch } = useMediaList({
    type: 'movie',
  ...
  const { groupedMovies, letterIndex } = useMemo(() => {
    if (!movies) return { groupedMovies: [], letterIndex: {} }
    ...
    movies.forEach((movie, idx) => {
      const firstChar = (movie.title || 'Unknown').charAt(0).toUpperCase()
    ...
    return { groupedMovies: groups, letterIndex: index }
  }, [movies])
  ...
  const renderItem = ({ item }: { item: MediaListItem }) => (
        ...
        {group.data.map((movieItem) => (
          <View key={movieItem.id} style={[styles.gridItem, { width: `${100 / numColumns}%`, padding: layout.cardGap / 2 }]}>
            <MediaCard
              ...
              onLongPress={() => handleItemLongPress(movieItem)}
              size="medium"
              showBadge
              showRating
              columns={numColumns}
