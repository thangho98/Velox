 ...
 *
 * Uses getPlatform() to get API base URL and device name.
 * Token callbacks are injected by the auth store.
 */
...
async function refreshAccessToken(): Promise<string | null> {
  ...
  try {
    ...
    const response = await fetch(`${platform.getApiBaseUrl()}/auth/refresh`, {
      method: 'POST',
...
async function fetchWithAuth<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  const platform = getPlatform()
  const url = `${platform.getApiBaseUrl()}${endpoint}`
  ...
  const deviceName = platform.getDeviceName()
  if (deviceName) {
  ...
  // Skip auto-refresh for auth endpoints that return 401 for business logic
