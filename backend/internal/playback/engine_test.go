...
func TestDecide(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "4 H.264+DTS MKV no-MKV profile → TranscodeAudio (Jellyfin-style: audio-only transcode)",
			media: MediaFileInfo{
		...
		{
			name: "5 H.264+DTS MP4 Chrome → TranscodeAudio (Jellyfin-style: audio-only transcode)",
			media: MediaFileInfo{
