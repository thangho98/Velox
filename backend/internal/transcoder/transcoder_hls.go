...
func (t *Transcoder) runHLSFFmpeg(ctx context.Context, inputPath, dir, prefix string, siIdx int, hdr bool, hwAccel string, videoCopy bool, startOffset float64, maxHeight int) error {
	...
	cmd := exec.CommandContext(ctx, ffmpegbin.FFmpeg(), args...)
...
// V2EncoderOpts configures a single-process HLS V2 encode session.
// Used by Session (stream_session.go) to launch per-session FFmpeg processes that produce
// separate video and audio HLS streams (one .m3u8 + .m4s set per track).
...
// StartHLSV2Encoder launches a single jellyfin-ffmpeg process that produces
// separate video and per-audio-track HLS fMP4 output streams. The process runs
// continuously until the context is cancelled or playback ends; callers use
// PrimeFromSegment to restart it at a new seek position.
