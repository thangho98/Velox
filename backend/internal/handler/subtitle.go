...
// Serve serves subtitle content as WebVTT.
func (h *SubtitleHandler) Serve(w http.ResponseWriter, r *http.Request) {
	...
	data, err := h.svc.ServeContent(r.Context(), subtitleID)
	if errors.Is(err, service.ErrNotFound) {
		respondError(w, http.StatusNotFound, "subtitle content not found")
		return
	}
	if err != nil {
		respondError(w, http.StatusInternalServerError, err.Error())
...
func (h *SubtitleHandler) Translate(w http.ResponseWriter, r *http.Request) {
	...
	sub, err := h.svc.Translate(r.Context(), subtitleID, req.TargetLanguage)
	if err != nil {
		respondError(w, http.StatusInternalServerError, err.Error())
		return
	}
